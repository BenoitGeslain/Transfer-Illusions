...
public class CSVSaver : MonoBehaviour {
	...
	public void writeContinousEntry(Trial trial, string time, int index, Vector3 positionR, Vector3 orientationR, Vector3 positionV, Vector3 orientationV, List<float> acceleration, int[] collisions, float score) {
		...
		write(new string[] {trial.parameters[0], trial.parameters[1], trial.parameters[2], trial.parameters[3], trial.parameters[4], time,
							...
							orientationV.x.ToString(), orientationV.y.ToString(), orientationV.z.ToString(), string.Join(";", acceleration),
							collisions[0].ToString(), collisions[1].ToString(), collisions[2].ToString(), collisions[3].ToString(), score.ToString()});
		writer.Flush();
		writer.Close();
	}

	public void writeDiscreteEntry(Trial trial, string timeCube, string timeTrial, int index, Vector3 positionError, float orientationError, List<int> hitCount, int[] collisions, float score) {
		writer = new StreamWriter(discretePath, true);

		write(new string[] {trial.parameters[0], trial.parameters[1], trial.parameters[2], trial.parameters[3], trial.parameters[4],
							timeCube, timeTrial, index.ToString(), positionError.x.ToString(), positionError.y.ToString(), positionError.z.ToString(),
							orientationError.ToString(), string.Join(";", hitCount), collisions[1].ToString(), collisions[2].ToString(),
							collisions[3].ToString(), score.ToString()});
		writer.Flush();
		writer.Close();
	}
	...
	public void writeConfig(string time, string cubeName, Vector3 positionR, Vector3 orientationR) {
		...
		write(new string[] {time, cubeName, positionR.x.ToString(), positionR.y.ToString(), positionR.z.ToString(),
